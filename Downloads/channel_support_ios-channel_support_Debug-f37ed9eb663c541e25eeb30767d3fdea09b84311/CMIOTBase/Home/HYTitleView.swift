...
class HYTitleView: UIView {
    ...
    fileprivate lazy var scrollLine: UIView = {
        ...
        scrollLine.addLineLayerWithColors(bounds: CGRect.init(x: 0, y: 0, width: 50, height: 1))
        return scrollLine
    }()
...
extension HYTitleView {
    ...
    private func setupTitleLabels() {
        ...
        for (index, title) in titles.enumerated() {
            ...
            label.text = title
            if(title == "网络用品"){
                label.font = titleStyle.selectedFont
            }else{
    ...
    private func setupTitleLabelsFrame() {
        ...
        for (index, label) in titleLabels.enumerated() {
            ...
            if titleStyle.isScrollEnable { // 可以滚动
                width = (label.text! as NSString).boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: 0), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: titleStyle.selectedFont], context: nil).size.width + 20
                if index == 0 {
...
extension HYTitleView {
    ...
    fileprivate func changeSelect(_ oldLabel: UILabel, _ newLabel: UILabel) {
        ...
        if titleStyle.isScrollEnable {
            var offsetX: CGFloat = newLabel.center.x - bounds.width * 0.8
            if offsetX < 0 {
                print("11")
                offsetX = 0
...
extension HYTitleView: HYContentViewDelegate {
    ...
    func contentView(_ contentView: HYContentView, originIndex: Int, targetIndex: Int, progress: CGFloat) {
        ...
        let targetLabel = titleLabels[Int(progress)]
        ...
        if titleStyle.hasGradient { // 标题文字渐变
            ...
            for label in titleLabels{
                label.font = UIFont.systemFont(ofSize: titleStyle.selectedFont.pointSize)
            }
            targetLabel.font = UIFont.boldSystemFont(ofSize: titleStyle.normalFont.pointSize + (fontDelta * progress))
        ...
        if titleStyle.lineScrollType == .RealTime { // 滚动条实时滚动
            ...
            if(progress > 1.0){
                let progress1:CGFloat = 1.0
              scrollLine.frame.origin.x = originLabel.frame.origin.x + xDelta * progress1
              scrollLine.frame.size.width = originLabel.frame.size.width + wDelta * progress1
            }
