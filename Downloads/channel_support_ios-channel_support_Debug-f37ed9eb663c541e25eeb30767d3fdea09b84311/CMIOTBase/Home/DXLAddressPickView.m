...
- (void)showWithProvince:(NSString *)province city:(NSString *)city county:(NSString *)county{
    ...
    ProvinceModel *model = nil;
    CityModel *citymodel = nil;
    AreaModel *teanmodel = nil;
    for (int i = 0; i < self.shengArray.count; i++) {
        ProvinceModel * selProvince = self.shengArray[i];
        if ([selProvince.name isEqualToString:province]) {
            model = selProvince;
            self.selectRowWithProvince = i;
            break;
        }
    }
    for (int i = 0; i < model.city.count; i++) {
        CityModel * selCity = model.city[i];
        if ([selCity.name isEqualToString:city]){
            citymodel = selCity;
            self.selectRowWithCity = i;
            break;
        }
        
    }
    for (int i = 0; i < citymodel.area.count; i++) {
        AreaModel * selArea = citymodel.area[i];
        if ([selArea.name isEqualToString:county]){
            teanmodel = selArea;
            self.selectRowWithTown = i;
            break;
        }
    }
    [self show];
    [self.pickerView selectRow:self.selectRowWithProvince inComponent:0 animated:false];
    [self.pickerView selectRow:self.selectRowWithCity inComponent:1 animated:false];
    [self.pickerView selectRow:self.selectRowWithTown inComponent:2 animated:false];
}
- (void)initView{
...
- (UIButton *)DetermineBtn
{
    if (!_DetermineBtn) {
        ...
        _DetermineBtn.titleLabel.font = [UIFont systemFontOfSize: 18.0];
...
- (void)determineBtnAction:(UIButton *)button
{
    ...
    if (self.determineBtnBlock) {
        self.determineBtnBlock(provincemodel.code, cityModel.code, areaModel.code, provincemodel.name, cityModel.name, areaModel.name);
    }
    [self dismiss];
}
