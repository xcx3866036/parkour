...
extension UIImage {
    ...
    /// 拼接多图
    public class func combinImages(images:[UIImage]) -> UIImage {
        var combinedIamge = images.first!
        for (index, image) in images.enumerated() {
            if index == 0 {continue}
            combinedIamge = UIImage.combinTwoImage(image1: combinedIamge, image2: image)
        }
        return combinedIamge
    }
    
    public class func combinTwoImage(image1:UIImage,image2:UIImage) -> UIImage{
