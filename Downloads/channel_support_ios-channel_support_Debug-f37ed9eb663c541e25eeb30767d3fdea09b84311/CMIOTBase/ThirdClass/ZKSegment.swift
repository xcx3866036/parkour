...
public class ZKSegment: UIScrollView {
    ...
    //MARK: 项更改事件
     var itemChange: ZKItemChange?
    //MARK: 每一项颜色
     var itemColor: UIColor!
    //MARK: 选中项颜色
     var itemSelectedColor: UIColor!
    //MARK: 选中项样式颜色
     var itemStyleSelectedColor: UIColor!
    //MARK: 每一项字体
     var itemFont: UIFont!
    //MARK: 每一项间距
    ...
    //MAKR: 样式
     ...
     var itemStyle: UIView!
     var itemStyleY: CGFloat = 0
     var itemStyleHeight: CGFloat = 0
    
    //MARK: .rectangle 样式圆角属性
     let itemRectangleStyleCornerRadius: CGFloat = 6
    //MARK: .rectangle 样式每项内间距
     let itemRectangleStylePadding: CGFloat = 8
     ...
     init(frame: CGRect,
        ...
        switch style {
        case .line:
            self.itemStyleY = self.h - 2
            self.itemStyleHeight = 2
        case .rectangle:
            self.itemStyleY = self.itemRectangleStylePadding
            self.itemStyleHeight = self.h - self.itemStyleY * 2
...
public extension ZKSegment {
    ...
    //MARK: 根据索引选中
    public func selectBy(_ index: Int) {
        if index >= 0 {
    ...
    public func remove(_ index: Int) {
        if index < 0 || index > self.items.count - 1 {
            log.error("ZKSegment ->>>>>> error: remove 方法索引不对")
            return
        }
...
extension ZKSegment {
     func itemStyleFrame(_ x: CGFloat, _ width: CGFloat) -> CGRect {
        var styleX = x
     ...
     func createItem(_ item: String) {
        ...
        let button = UIButton(frame: CGRect(x: self.contentX, y: 0, width: itemWidth, height: self.h))
        ...
        if self.buttonSelected == nil {
            ...
            self.itemStyle.frame = self.itemStyleFrame(button.x, button.w)
    ...
    @objc  func itemClick(button: UIButton) {
     ...
     func animateChage(_ button: UIButton) {
        UIView.animate(
            withDuration: 0.3,
            delay: 0,
            ...
            UIView.animate(withDuration: 0.3, animations: {
                //MARK: 移动到中间
                if self.contentSize.width > self.w &&                                               // 内容宽度大于控件宽度
                    button.x > self.w / 2 - button.w / 2 &&                                     // 按钮的坐标大于屏幕中间位置
                    self.contentSize.width > button.x + self.w / 2 + button.w / 2 {             //内容的宽度大于按钮移动到中间坐标加上屏幕一半宽度加上按钮一半宽度
                    self.contentOffset = CGPoint(
                        x: button.x - self.w / 2 + button.w / 2,
                        y: 0)
                } else if button.x < self.w / 2 - button.w / 2 {
                     // 移动到开始
//                    self.contentOffset = CGPoint(x: 0, y: 0)
                } else if self.contentSize.width - button.x < self.w / 2 + button.w / 2 ||      // 内容宽度减去按钮的坐标小于屏幕的一半，移动到最后
                    button.x + button.w + self.itemMargin == self.contentSize.width {
                    if self.contentSize.width > self.w {
                         // 移动到末尾
//                        self.contentOffset = CGPoint(x: self.contentSize.width - self.width, y: 0)
                    }
                }
            })
        })
    ...
    //MARK: 根据样式获取项宽度
     func itemWidth(_ item: String) -> CGFloat {
    ...
    //MARK: 如果内容宽度小于控件宽度，居中显示
     func fiexItems() {
        if (self.w - self.contentX > self.itemMargin) {
            var bigItemSumWidth: CGFloat = 0
            var bigItemCount: CGFloat = 0
            self.contentX = self.itemMargin
            // 计算平均每项宽度
            var itemWidth = (self.w - (CGFloat(self.buttons.count) + 1) * self.itemMargin) / CGFloat(self.buttons.count)
            // 检查是否有超过平均宽度的项
            self.buttons.forEach({ button in
                if button.w > itemWidth {
                    bigItemCount += 1
                    bigItemSumWidth += button.w
                }
            })
            // 减去超过平均宽度项的宽度总和，重新计算剩余项的宽度
            itemWidth = (self.w - (CGFloat(self.buttons.count) + 1) * self.itemMargin - bigItemSumWidth) / (CGFloat(self.buttons.count) - bigItemCount)
            // 重新布局
            self.buttons.forEach({ button in
                // 如果小于平均宽度，设置为平均宽度
                if button.w < itemWidth {
                    button.frame = CGRect(x: self.contentX, y: 0, width: itemWidth, height: self.h)
                    self.contentX += (itemWidth + self.itemMargin)
                } else {
                    button.frame = CGRect(x: self.contentX, y: 0, width: button.w, height: self.h);
                    self.contentX += (button.w + self.itemMargin)
                }
